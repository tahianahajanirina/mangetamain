...
def main():
    ...
    model_files = list(OUTPUTS_DIR.glob("**/*.pkl")) + list(OUTPUTS_DIR.glob("**/*.safetensors"))
    if not model_files:
    ...
    if not repo_id:
        repo_id = input("Enter Hugging Face repo ID (e.g., username/mangetamain-models): ").strip()
    ...
    if success:
        ...
        logger.info(f"  3. Download with: python scripts/download_models.py")
        return 0
    else:
        logger.error("=" * 80)
