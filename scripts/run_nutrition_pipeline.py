...
from src.feature_engineering.nutrition_classification_features import NutritionClassificationFeatures
...
def run_feature_engineering(skip: bool = False) -> str:
    ...
    if skip:
        if Path(features_path).exists():
            ...
            logger.warning(f"Fichier {features_path} introuvable, exécution du feature engineering...")
...
def evaluate_model(classifier: NutritionClassifier) -> dict:
    ...
    metrics = classifier.evaluate(classifier.X_test, classifier.y_test, dataset_name="Test")
...
def print_summary(
    ...
    for class_name, class_metrics in metrics['per_class'].items():
        print(f"  {class_name:15} - F1: {class_metrics['f1']:.4f}, "
