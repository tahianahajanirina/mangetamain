...
class RecipeRecommender:
    ...
    def __init__(
        ...
        logger.info(
            f"RecipeRecommender initialized with {len(self.recipes_df)} recipes"
        )
    ...
    def recommend_by_time(
        self, available_minutes: int, max_recipes: int = 10, time_buffer: int = 10
    ) -> pd.DataFrame:
        ...
        filtered["time_score"] = available_minutes - abs(
            filtered["predicted_time"] - available_minutes * 0.8
        )
        ...
        return recommendations[
            ["name", "predicted_time", "minutes", "n_steps", "n_ingredients", "tags"]
    ...
    def recommend_advanced(
        ...
        if dietary_tags:
            for tag in dietary_tags:
                ...
                filtered = filtered[
                    filtered["tags"].apply(
                        lambda x: (
                            any(tag_lower in str(t).lower() for t in x)
        ...
        if course_type:
            filtered = filtered[
                filtered["tags"].apply(
                    lambda x: (
                        any(course_type.lower() in str(t).lower() for t in x)
                        if isinstance(x, list)
                        else False
                    )
                )
        ...
        if difficulty == "easy":
            filtered = filtered[
                (filtered["n_steps"] <= 10) & (filtered["n_ingredients"] <= 12)
            ]
            ...
            for ingredient in exclude_ingredients:
                ...
                filtered = filtered[
                    ~filtered["ingredients"].apply(
                        lambda x: (
                            any(ingredient_lower in str(ing).lower() for ing in x)
                            if isinstance(x, list)
                            else False
                        )
                    )
    ...
    def get_statistics(self) -> Dict:
        ...
        return {
            ...
            "time_ranges": {
                "quick (< 30 min)": len(
                    self.recipes_df[self.recipes_df["predicted_time"] < 30]
                ),
                "moderate (30-60 min)": len(
                    self.recipes_df[
                ...
                "long (60+ min)": len(
                    self.recipes_df[self.recipes_df["predicted_time"] >= 60]
                ),
            },
