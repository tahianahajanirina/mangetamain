...
class IntegratedRecommendationPipeline:
    ...
    def _load_models(self):
        ...
        if time_model_path.exists():
            ...
            logger.info(
                "⚠ Time prediction model not found (will skip time predictions)"
            )
        ...
        for tag in nutrition_tags:
            ...
            if model_path.exists():
                try:
                    self.nutrition_models[tag] = NutritionTaggerModel.load_model(
                        model_path
                    )
        ...
        if cluster_models:
            try:
                ...
                self.recipe_clusterer = RecipeClusterer.load_model(
                    model_name, str(self.models_dir)
                )
    ...
    def train_svd_recommender(self, k: int = 50):
        ...
        if not self.interactions_path.exists():
            raise FileNotFoundError(
                f"Interactions file not found: {self.interactions_path}"
            )
        ...
        self.svd_recommender.fit(
            str(self.interactions_path), str(self.recipes_path), k=k
        )
    ...
    def get_recommendations(
        ...
        if not self.svd_trained:
            raise RuntimeError(
                "SVD recommender not trained. Call train_svd_recommender() first."
            )
        ...
        if len(recipes) > 0:
            ...
            recipes = recipes.merge(
                result["recommendations"][["id"]]
                .reset_index()
                .rename(columns={"index": "svd_rank"}),
                on="id",
                how="left",
        ...
        logger.info(
            f"✓ Returned {len(result['recommendations'])} enriched recommendations"
        )
    ...
    def _add_time_predictions(self, recipes: pd.DataFrame) -> pd.DataFrame:
        ...
        try:
            ...
            recipes_with_features = self.data_loader.prepare_for_time_prediction(
                recipes
            )
            ...
            available_features = [
                f for f in feature_names if f in recipes_with_features.columns
            ]

            if len(available_features) < len(feature_names):
                logger.warning(
                    f"Missing features for time prediction: {set(feature_names) - set(available_features)}"
                )
    ...
    def _add_nutrition_tags(self, recipes: pd.DataFrame) -> pd.DataFrame:
        ...
        try:
            ...
            recipes_with_features = self.data_loader.prepare_for_nutrition_tagging(
                recipes
            )
            ...
            for tag_name, model in self.nutrition_models.items():
                ...
                if len(available_features) < len(feature_names):
                    logger.warning(
                        f"Missing features for {tag_name}: {set(feature_names) - set(available_features)}"
                    )
                    continue
    ...
    def _add_clusters(self, recipes: pd.DataFrame) -> pd.DataFrame:
        ...
        try:
            ...
            if not all(f in recipes.columns for f in required_features):
                logger.warning(
                    "Required features for clustering not available. Skipping clustering."
                )
                return recipes
    ...
    def _apply_filters(self, recipes: pd.DataFrame, filters: Dict) -> pd.DataFrame:
        ...
        if "max_time" in filters and "predicted_time" in recipes.columns:
            ...
            logger.info(
                f"Filtered by time <= {max_time} minutes: {len(recipes)} recipes remain"
            )
        ...
        if "cluster_name" in filters and "cluster_name" in recipes.columns:
            ...
            logger.info(
                f"Filtered by cluster '{cluster}': {len(recipes)} recipes remain"
            )
    ...
    def analyze_recipe_sentiment(self, recipe_id: int, limit: int = 10) -> Dict:
        ...
        for _, row in recipe_reviews.iterrows():
            ...
            if pd.notna(review_text) and len(str(review_text).strip()) > 0:
                try:
                    label, confidence = SentimentAnalyzer.predict_sentiment(
                        str(review_text)
                    )
                    sentiments.append(
                        {
    ...
    def get_pipeline_status(self) -> Dict:
        ...
        return {
            ...
            "recipe_clusterer": (
                "loaded" if self.recipe_clusterer is not None else "not available"
            ),
