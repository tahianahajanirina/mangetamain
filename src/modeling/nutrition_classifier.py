...
class NutritionClassifier:
    ...
    def load_data(self, filepath: str) -> Tuple[pd.DataFrame, pd.Series]:
        ...
        feature_cols = [
            ...
            'is_low_sodium', 'is_low_sugar',
            
            # Complexité et interactions
            'complexity_score', 'calorie_protein_interaction', 
        ...
        for cat_id, count in y.value_counts().sort_index().items():
            pct = (count / len(y)) * 100
            logger.info(f"    {cat_id} - {self.class_names[cat_id]}: {count:,} ({pct:.1f}%)")
    ...
    def scale_features(
        ...
        logger.info(f"  ✓ Features normalisées (μ=0, σ=1)")
    ...
    def evaluate(
        ...
        logger.info("\n  Matrice de Confusion:")
        logger.info(f"  {'':15} " + " ".join(f"{name:>12}" for name in self.class_names))
        for i, row in enumerate(cm):
            logger.info(f"  {self.class_names[i]:15} " + " ".join(f"{val:>12}" for val in row))
