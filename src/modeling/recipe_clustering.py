...
class RecipeClusterer:
    ...
    def __init__(
        self, n_clusters: int = None, pca_variance: float = 0.90, random_state: int = 42
    ):
        """
        Args:
    ...
    def prepare_features(self) -> Tuple[np.ndarray, np.ndarray]:
        ...
        logger.info(
            "Solution: Cluster par temps/complexité/santé, puis filtrer par popularité.\n"
        )
    ...
    def find_optimal_k(
        self, k_range: range = range(3, 11), sample_size: int = 20000
    ) -> int:
    ...
    def fit(self, features_path: str = None) -> "RecipeClusterer":
        ...
        final_silhouette = silhouette_score(
            self.features_pca[sample_for_sil], clusters[sample_for_sil]
        )
    ...
    def plot_clusters(self, output_dir: str = None) -> None:
        ...
        ax.set_title(
            f"Clustering V3 OPTIMISÉ (K={self.n_clusters}, Silhouette={final_silhouette:.3f})",
            fontsize=14,
            fontweight="bold",
        )
