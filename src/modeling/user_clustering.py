...
class UserClusterer:
    ...
    def __init__(self, n_clusters: int = None, pca_variance: float = 0.95, 
                 random_state: int = 42):
        """
        Args:
    ...
    def prepare_features(self) -> Tuple[np.ndarray, np.ndarray]:
        ...
        feature_cols = [col for col in self.users.columns 
    ...
    def fit(self, features_path: str = None) -> 'UserClusterer':
        ...
        for cluster_id in range(self.n_clusters):
            ...
            logger.info(f"  Cluster {cluster_id}: {count:>7,} utilisateurs ({pct:>5.1f}%)")
    ...
    def name_clusters(self) -> Dict[int, str]:
        ...
        key_features = ['avg_rating', 'avg_minutes', 'avg_calories', 
                       'avg_n_steps', 'n_interactions']
    ...
    def plot_clusters(self, output_dir: str = None) -> None:
        ...
        for cluster_id in range(self.n_clusters):
            ...
            ax.scatter(
                ...
                alpha=0.6, s=30, color=colors[cluster_id], edgecolors='none'
            )
        ...
        ax.set_ylabel(f'PC2 ({pca_viz.explained_variance_ratio_[1]:.1%})',
                     fontsize=12, fontweight='bold')
        ax.set_title(f'Clustering Utilisateurs (K={self.n_clusters}, Silhouette={final_silhouette:.3f})',
