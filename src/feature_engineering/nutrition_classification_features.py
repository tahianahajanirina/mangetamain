...
class NutritionClassificationFeatures:
    ...
    def calculate_pdv_percentages(self, df: pd.DataFrame) -> pd.DataFrame:
        ...
        df['saturated_fat_pdv'] = (df['saturated_fat'] / self.daily_values['saturated_fat']) * 100
        df['sodium_pdv'] = (df['sodium'] / self.daily_values['sodium']) * 100
        df['carbs_pdv'] = (df['carbohydrates'] / self.daily_values['carbohydrates']) * 100
    ...
    def calculate_nutritional_ratios(self, df: pd.DataFrame) -> pd.DataFrame:
        ...
        df['saturated_fat_ratio'] = np.where(
            df['total_fat'] > 0,
            (df['saturated_fat'] / df['total_fat']) * 100,
            0
        )
    ...
    def categorize_nutrition(self, df: pd.DataFrame) -> pd.DataFrame:
        ...
        very_healthy_mask = (
            ...
            (df['sodium_pdv'] <= self.health_thresholds['very_healthy']['sodium_pdv_max']) &
            (df['sugar_pdv'] <= self.health_thresholds['very_healthy']['sugar_pdv_max']) &
            (df['protein_pdv'] >= self.health_thresholds['very_healthy']['protein_pdv_min'])
        )
    ...
    def build_features(self, input_path: str, output_path: str) -> Tuple[pd.DataFrame, Dict]:
        """
        ...
        feature_cols = [
            ...
            # PDV%
            'calories_pdv', 'fat_pdv', 'saturated_fat_pdv', 'sodium_pdv',
            'carbs_pdv', 'sugar_pdv', 'protein_pdv',
            
            # Ratios
            'protein_density', 'sugar_ratio', 'sodium_density',
            'saturated_fat_ratio', 'macro_balance',
            
            # Score et catégorie
            'health_score', 'nutrition_category',
            
            # Features additionnelles
            'is_low_calorie', 'is_high_protein', 'is_low_fat',
            'is_low_sodium', 'is_low_sugar', 'complexity_score',
            'calorie_protein_interaction', 'fat_sodium_interaction',
    ...
    def _generate_statistics(self, df: pd.DataFrame) -> Dict:
        ...
        stats = {
            ...
            'feature_correlations': {
                'health_score_calories': float(df[['health_score', 'calories']].corr().iloc[0, 1]),
                'health_score_protein': float(df[['health_score', 'protein_pdv']].corr().iloc[0, 1]),
                'health_score_sugar': float(df[['health_score', 'sugar_pdv']].corr().iloc[0, 1]),
            }
        }
        return stats
