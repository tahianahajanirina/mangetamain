...
class TimeFeatureEngineer:
    ...
    def __init__(
        self,
        cooking_verbs: Set[str],
        equipment_terms: Set[str]
    ):
    ...
    def create_step_features(self, df: pd.DataFrame) -> pd.DataFrame:
        ...
        df['time_mentions_count'] = df['steps_text'].apply(
            lambda text: len(re.findall(r'\d+\s*(?:minute|min|hour|hr)s?', text, re.IGNORECASE))
        )
        ...
        df['has_multiple_stages'] = df['steps_text'].str.contains(
            'meanwhile|while|during', case=False
        ).astype(int)

        df['has_waiting_time'] = df['steps_text'].str.contains(
            'rest|chill|refrigerate|freeze|marinate|cool|set|rise',
            case=False
        ).astype(int)

        df['has_preheating'] = df['steps_text'].str.contains(
            'preheat', case=False
        ).astype(int)

        # Cooking method features
        df['has_baking'] = df['steps_text'].str.contains(
            r'\bbake\b|\boven\b', case=False
        ).astype(int)

        df['has_simmering'] = df['steps_text'].str.contains(
            r'\bsimmer\b|\bboil\b', case=False
        ).astype(int)

        df['has_frying'] = df['steps_text'].str.contains(
            r'\bfry\b|\bfrying\b|\bdeep.?fry\b', case=False
        ).astype(int)

        df['has_grilling'] = df['steps_text'].str.contains(
            r'\bgrill\b|\bbroil\b|\bbarbecue\b|\bbbq\b', case=False
        ).astype(int)

        df['has_slow_cooking'] = df['steps_text'].str.contains(
            r'slow.?cooker|crock.?pot|slow.?cook', case=False
        ).astype(int)

        # Temperature features
        df['has_temperature'] = df['steps_text'].str.contains(
            r'\d+\s*degrees|°[fc]|\d+°', case=False
        ).astype(int)
    ...
    def create_tag_features(self, df: pd.DataFrame) -> pd.DataFrame:
        ...
        df['tags_text'] = df['tags'].apply(
            lambda tags: ' '.join(tags).lower() if isinstance(tags, list) else str(tags).lower()
        )
        ...
        def extract_tag_time_limit(tags_text):
            ...
            minutes_match = re.search(r'(\d+)[-\s]*minutes?[-\s]*or[-\s]*less', tags_text)
            if minutes_match:
        ...
        df['is_appetizer'] = df['tags_text'].str.contains('appetizer|snack', case=False).astype(int)
        df['is_dessert'] = df['tags_text'].str.contains('dessert|sweet', case=False).astype(int)
        df['is_main_dish'] = df['tags_text'].str.contains('main-dish|main-ingredient', case=False).astype(int)
        df['is_beverage'] = df['tags_text'].str.contains('beverage|drink', case=False).astype(int)

        # Difficulty indicators
        df['is_easy'] = df['tags_text'].str.contains('easy|beginner|simple', case=False).astype(int)
    ...
    def create_name_features(self, df: pd.DataFrame) -> pd.DataFrame:
        ...
        df['name_has_quick'] = df['name_lower'].str.contains(
            'quick|fast|minute|rapid|easy', case=False
        ).astype(int)

        # Slow cooking indicators
        df['name_has_slow'] = df['name_lower'].str.contains(
            'slow|crockpot|overnight', case=False
        ).astype(int)
    ...
    def get_feature_names(self) -> list:
        ...
        return [
            # Basic counts
            'n_steps', 'n_ingredients', 'steps_word_count',

            # Explicit time features (MOST IMPORTANT)
            'total_explicit_time', 'has_explicit_time', 'time_mentions_count',
            'tag_time_limit', 'has_time_tag',

            # Cooking methods
            'cooking_verb_count', 'equipment_count',
            'has_baking', 'has_simmering', 'has_frying', 'has_grilling', 'has_slow_cooking',

            # Temperature
            'has_temperature', 'max_temperature',

            # Complexity
            'avg_step_length', 'avg_ingredient_length', 'steps_per_ingredient',
            'step_ingredient_complexity',

            # Time indicators
            'has_multiple_stages', 'has_waiting_time', 'has_preheating',

            # Course and difficulty
            'is_appetizer', 'is_dessert', 'is_main_dish', 'is_beverage', 'is_easy',

            # Name features
            'name_length', 'name_word_count', 'name_has_quick', 'name_has_slow'
        ]
