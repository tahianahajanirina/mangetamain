...
class UserFeatureBuilder:
    ...
    def load_data(
        self, recipes_path: str, interactions_path: str
    ...
    def merge_data(
        ...
        merged = interactions.merge(
            recipes, left_on="recipe_id", right_on="id", how="left"
        )
    ...
    def build_behavioral_features(self, merged: pd.DataFrame) -> pd.DataFrame:
        ...
        behavioral = (
            ...
            .agg(
                {"recipe_id": "count", "rating": "mean"}  # n_interactions  # avg_rating
            )
            .rename(columns={"recipe_id": "n_interactions", "rating": "avg_rating"})
        )
        ...
        for category, tags in tag_categories.items():
            ...
            for user_id in tag_features.index:
                ...
                if "tags_parsed" in user_interactions.columns:
                    count = (
                        user_interactions["tags_parsed"]
                        .apply(
                            lambda x: (
                                any(tag in str(x).lower() for tag in tags)
                                if pd.notna(x)
                                else False
                            )
                        )
    ...
    def build_features(self, recipes_path: str, interactions_path: str) -> pd.DataFrame:
        ...
        logger.info(
            f"Filtrage utilisateurs (min {self.min_interactions} interactions)..."
        )
        initial_count = len(users_profiles)
        users_profiles = users_profiles[
            users_profiles["n_interactions"] >= self.min_interactions
        ]
        logger.info(
            f"Utilisateurs conservés: {len(users_profiles):,} / {initial_count:,}"
        )
        ...
        logger.info("=" * 80)
        logger.info("FEATURES UTILISATEURS CRÉÉES")
