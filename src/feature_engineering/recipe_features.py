...
class RecipeFeatureBuilder:
    ...
    def calculate_popularity_metrics(self, recipes: pd.DataFrame, 
        ...
        # Agréger par recette
        interactions_agg = interactions.groupby('recipe_id').agg({
            'user_id': 'count',
            'rating': 'mean'
        }).rename(columns={
    ...
    def build_features(self, recipes_path: str, interactions_path: str) -> pd.DataFrame:
        ...
        recipes_filtered['log_interactions'] = np.log1p(recipes_filtered['n_interactions'])
        ...
        recipes_filtered['efficiency'] = (
            recipes_filtered['popularity_score'] / (1 + recipes_filtered['log_minutes'])
        )
        ...
        features_cols = [
            'id', 'name',
            'popularity_score', 'log_minutes', 'time_complexity', 
...
def build_recipe_features(recipes_path: str, 
